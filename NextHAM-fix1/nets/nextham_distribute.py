...
class GraphAttentionTransformerHAM(torch.nn.Module):
    def __init__(self,
        ...
        # exit(0)
        self.input_rs = LinearRS(self.irreps_edge_output, o3.Irreps('32x0e+16x1o+16x1e+8x2e+8x2o+8x3e+8x3o+8x4e'), bias=False)
    ...
    @torch.enable_grad()
    def forward(self, weak_ham_in, node_num, edge_src, edge_dst, edge_vec, batch, node_atom, edge_mask_list, node_mask_list, use_sep = True, range_dis = [0.0, 10.0]):
        ...
        for blk_idx, blk in enumerate(self.blocks):
            node_features_slice_list = []
            edge_features_slice_list = []
            next_node_features = None
            next_edge_features = None
            for mask_idx in range(len(edge_mask_list)):
                edge_mask = edge_mask_list[mask_idx]
                node_mask = node_mask_list[mask_idx]
                edge_src_slice = edge_src[edge_mask]
                edge_dst_slice = edge_dst[edge_mask]
                edge_features_slice = edge_features[edge_mask]
                print(edge_features.shape, edge_features_slice.shape)
                edge_sh_slice = edge_sh[edge_mask]
                edge_length_embedding_slice = edge_length_embedding[edge_mask]
                edge_length_slice = edge_length[edge_mask]
                node_features_slice, edge_features_slice = blk(node_input=node_features, node_attr=node_attr, 
                    edge_src=edge_src_slice, edge_dst=edge_dst_slice, edge_attr=edge_features_slice, edge_sh = edge_sh_slice, 
                    edge_scalars=edge_length_embedding_slice, 
                    batch=batch, edge_length=edge_length_slice)
                if next_edge_features is None:
                    next_edge_features = torch.zeros((edge_features.shape[0], edge_features_slice.shape[-1]))
                next_edge_features[edge_mask] = edge_features_slice
                if next_node_features is None:
                    next_node_features = torch.zeros((node_features.shape[0], node_features_slice.shape[-1]))
                next_node_features[node_mask] = node_features_slice[node_mask]
            node_features = next_node_features
            edge_features = next_edge_features
            ...
            if self.with_trace:
                if blk_idx >= self.start_layer:
                    if blk_idx < len(self.blocks)-1:
                        ...
                        edge_features = edge_features + edge_features_equi_nonlin  
                    else:
        ...
        if self.with_trace:
            ...
            edge_trace = 0.
        if use_sep:
            edge_ham = edge_ham * mask_num * torch.exp(-0.2*edge_length)[:, None]  
            edge_trace = edge_trace * mask_num
        return edge_ham, edge_trace, mask_num
...
def graph_attention_transformer_nonlinear_materials_ham_soc(irreps_in, irreps_edge, radius, num_basis=64, 
    ...
    model = GraphAttentionTransformerHAM(
        ...
        irreps_node_embedding='32x0e+16x1e+16x1o+8x2e+8x2o+8x3e+8x3o+8x4e+8x4o+1x5o+1x6e',  
        irreps_edge_embedding='32x0e+16x1e+16x1o+8x2e+8x2o+8x3e+8x3o+8x4e+8x4o+1x5o+1x6e', num_layers=4, start_layer=start_layer,
