...
├── ...
    ...
    def process_worker(self, folder, **kwargs):
        ...
        file_path = os.path.join(folder, "energy.dat")
        if os.path.exists(file_path):
            with open(file_path, 'r') as file:
                energy = torch.tensor([float(i) for i in file.read().split()], dtype=self.default_dtype_torch)
        else:
            energy = None
        
        file_path = os.path.join(folder, "force.dat")
        if os.path.exists(file_path):
            with open(file_path, 'r') as file:
                force = torch.tensor([float(i) for i in file.read().split()], dtype=self.default_dtype_torch).reshape(-1,3)
        else:
            force = None
        ...
        return get_graph(cart_coords, frac_coords, numbers, stru_id, r=self.radius, max_num_nbr=self.max_num_nbr,
                         ...
                         only_ij=self.only_ij, create_from_DFT=self.create_from_DFT, energy = energy, 
                         force = force, zstar = None, **kwargs)
