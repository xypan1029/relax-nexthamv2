...
last_angles = torch.zeros(3)

def random_angle(tensor, mean=0, std=1, trunc_min=-2, trunc_max=2):
    with torch.no_grad():
        while True:
            tensor.normal_(mean=mean, std=std)
            cond = torch.logical_or(tensor < trunc_min, tensor > trunc_max)
            if not cond.any():
                break
    tensor.clamp_(min=trunc_min, max=trunc_max)
    return tensor

def get_random_R(alpha=0.3, do_strong_aug=False):
    global last_angles
    angles = torch.empty(3)
    if do_strong_aug:
        random_angle(angles, mean=0, std=1, trunc_min=-0.5, trunc_max=0.5)
    else:
        random_angle(angles, mean=0, std=1, trunc_min=-0.2, trunc_max=0.2)

    angles = alpha * angles + (1 - alpha) * last_angles
    # print(do_strong_aug, angles)
    last_angles = angles  
    ...
    Ry = torch.tensor([[cos[1], 0, sin[1]],
                       [0, 1, 0],
                       [-sin[1], 0, cos[1]]])

    Rz = torch.tensor([[cos[2], -sin[2], 0],
                       [sin[2], cos[2], 0],
                       [0, 0, 1]])
...
class Rotate:
    def __init__(self, default_dtype_torch, device_torch='cpu', spinful=False):
        ...
        self.Us_openmx2wiki = {
            ...
            # 1: torch.eye(3, dtype=default_dtype_torch)[[2, 0, 1]].to(device=device_torch), #[0,1,-1]->[-1,0,1]
            2: torch.eye(5, dtype=default_dtype_torch)[[2, 4, 0, 3, 1]].to(device=device_torch),
            # 2: torch.eye(5, dtype=default_dtype_torch)[[4, 2, 0, 1, 3]].to(device=device_torch), #[0,1,-1,2,-2]->[-2,-1,0,1,2]
            3: torch.eye(7, dtype=default_dtype_torch)[[6, 4, 2, 0, 1, 3, 5]].to(device=device_torch), #[0,1,-1,2,-2,3,-3]->[-3,-2,-1,0,1,2,3]
            # 4: torch.eye(9, dtype=default_dtype_torch)[[8, 6, 4, 2, 0, 1, 3, 5, 7]].to(device=device_torch),
            # 5: torch.eye(11, dtype=default_dtype_torch)[[10, 8, 6, 4, 2, 0, 1, 3, 5, 7, 9]].to(device=device_torch),
            # 6: torch.eye(13, dtype=default_dtype_torch)[[12, 10, 8, 6, 4, 2, 0, 1, 3, 5, 7, 9, 11]].to(device=device_torch)
        }
...
class e3TensorDecomp:
    ...
    def get_H_trace(self, H):
        ...
        for i in range(len(self.out_js_list)):
            ...
            if self.spinful:
                ...
                H_block_trace = torch.sum(H_block.abs().square(), dim=(-1, -2, -3))
            else:
                H_block = H[:, H_slice].reshape(-1, 2 * l1 + 1, 2 * l2 + 1)
                H_block_trace = torch.sum(H_block.abs().square(), dim=(-1, -2))
            out.append(H_block_trace)
        out = torch.stack(out, dim = -1)
        out = F.layer_norm(out, normalized_shape=(out.shape[-1],))
        return out

    def mul_by_norm(self, H, norm_tensor):
        assert len(self.out_js_list) == len(norm_tensor[-1])
        H_new = H.clone()
        for i in range(len(self.out_js_list)):
            H_slice = slice(self.H_slices[i], self.H_slices[i + 1])
            if self.spinful:
                H_new[:, :, H_slice] = H[:, :, H_slice] * norm_tensor[:, :, i][:,:,None]
            else:
                H_new[:, H_slice] = H[:, H_slice] * norm_tensor[:, i][:,None]
        return H_new

    def mask_high_degree(self, H_mask, l_max=2):
        H_mask_new = H_mask.clone()
        for i in range(len(self.out_js_list)):
            H_slice = slice(self.H_slices[i], self.H_slices[i + 1])
            l1, l2 = self.out_js_list[i]
            if l1 > l_max or l2 > l_max:
                if self.spinful:
                    H_mask_new[:, :, H_slice] = torch.zeros_like(H_mask[:, :, H_slice])
                else:
                    H_mask_new[:, H_slice] = torch.zeros_like(H_mask[:, H_slice])
        return H_mask_new
    ...
    def get_net_out(self, H):
        ...
        out = []
        # print(len(self.out_js_list))
        # print(self.out_js_list)
        for i in range(len(self.out_js_list)):
            H_slice = slice(self.H_slices[i], self.H_slices[i + 1])
            l1, l2 = self.out_js_list[i]
            if self.spinful:
...
class e3ElementWise:
    ...
    def __call__(self, x: torch.Tensor, weight: torch.Tensor):
        ...
        for mul, ir in self.irreps_in:
            ...
            field = field.reshape(*field.shape[:2], mul, ir.dim)
