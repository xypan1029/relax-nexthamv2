...
class TrainConfig(BaseConfig):
    def __init__(self, config_file):
        ...
        if self.checkpoint:
            assert self.checkpoint.endswith('.pkl'), 'checkpoint should point to model_last_epoch.pkl or model_best_epoch_by_val.pkl'
            config1_path = os.path.join(os.path.dirname(self.checkpoint), 'src/train.ini')
    ...
    def get_basic_section(self):
        ...
        self.checkpoint = self._config.get('basic', 'checkpoint')
        self.simp_out = self._config.getboolean('basic', 'simplified_output')
        self.do_strong_aug = self._config.getboolean('basic', 'do_strong_aug')
        self.use_new_hypp = self._config.getboolean('basic', 'use_new_hypp')
        self.train_target = self._config.get('basic', 'train_target')
    ...
    def get_network_section(self):
        ...
        self._trace_out_len = self._config.get('network', 'trace_out_len')
    ...
    @property
    def trace_out_len(self):
        return self._trace_out_len
