...
class RevertDecayLR:
    ...
    def revert(self):
        best_checkpoint = torch.load(os.path.join(self.save_model_dir, 'model_best_epoch_by_val.pkl'))
    ...
    def save_model(self, epoch, val_loss, is_best=False, is_best_with_aug=False, **kwargs):
        state = {
            'epoch': epoch,
            'val_loss': val_loss,
        ...
        state.update(kwargs)
        if is_best:
            # print('save model_best_epoch_by_val.pkl')
            torch.save(state, os.path.join(self.save_model_dir, 'model_best_epoch_by_val.pkl'))
        # print('save model_last_epoch.pkl')
        torch.save(state, os.path.join(self.save_model_dir, 'model_last_epoch.pkl'))

def load_trained_ckpt(configs):
    return torch.load(os.path.join(configs.save_dir, 'model_best_epoch_by_val.pkl'), map_location=configs.device)            

def process_targets(orbital_types, index_to_Z, targets): 
    Z_to_index = torch.full((118,), -1, dtype=torch.int64)
