...
train_info_recorder = namedtuple('train_info', ['epoch', 'global_step', 'lr', 'total_time', 'epoch_time', 'best_loss', 'train_losses', 'val_losses', 'val_loss_list', 'extra_val', 'extra_val_list', 'energy_losses', ])
...
class DeepHE3Kernel:
    ...
    def train(self, train_config):
        ...
        # = Build net =
        print('\n------- Build model -------')
        if config.checkpoint_dir and False:
        ...
        if config.checkpoint_dir:
            ...
            global_step = checkpoint["epoch"] + 1
        ...
        try:
            while epoch < config.num_epoch and learning_rate > config.min_lr:
                ...
                train_losses, _, energy_losses = self.get_loss(train_loader, is_train=True)
                
                #with torch.no_grad():
                    # = val =
                val_losses, val_loss_list, _ = self.get_loss(val_loader)
        ...
        if os.path.isfile(test_h5):
            ...
            os.remove(test_h5)
        #with torch.no_grad():
        test_losses, test_loss_list, _ = self.get_loss(test_loader, save_h5=True)
    ...
    def get_graph(self, config: TrainConfig, inference=False): # todo: dataset info stored separately
    ...
    def get_nodetarget(self, output_node, train_target, data):
        if train_target == 'E':
            return output_node.reshape(-1), None, None
            #return output_node, None, None

        if train_target == 'EF':
            mask = data["edge_attr"][:,0] != 0
            force_edge0 = torch.autograd.grad(torch.sum(output_node), data["edge_attr"], create_graph=True, only_inputs=True, allow_unused=True)[0][mask]
            ...
            #        continue
            #    force[edge_index[0][i]] -= force_edge[i]
            #    force[edge_index[1][i]] += force_edge[i]
            ...
            return output_node.reshape(-1), force, None

        if train_target == 'Z':
            wm = []
            for i in range(2):
                wm.append(wigner_3j(1, 1, i, dtype=output_node.type, device=output_node.device))
            wm = torch.cat(wm, dim = -1)
            Z = torch.sum(wm[None, :, :, :] * output_node[:, None, None, :], dim=-1)
            return None, None, Z
    
    def generate_random_rotation_matrix(self, dim=3):
    ...
    def get_loss(self, loader, is_train=False, save_h5=False):
        ...
        for batch in loader:
            ...
            #R = self.generate_random_rotation_matrix()
            #R = torch.eye(3)
            #data["edge_attr"][:, [1,2,3]] = data["edge_attr"][:, [1,2,3]] @ R.to(config.device)
            data["edge_attr"].requires_grad = True
            t0 = time.time()
            output_node, output_edge = net(data)
            if torch.isnan(output_edge).any():
                print(data.stru_id)
            #if config.convert_net_out:
                #H_pred = output_edge
            #else:
            #    H_pred = construct_kernel.get_H(output_edge)
            if config.train_target is not None:
                Energy, Force, Zstar = self.get_nodetarget(output_node, config.train_target, data)
            #Force = Force @ R.T.to(config.device)
            # get loss
            #loss = criterion(H_pred, batch.label.to(device=config.device), batch.mask)
            #print(Energy, data.energy, '\n', Force, data.force)
            if config.train_target is not None:
                if Energy is not None:
                    energy_loss = criterion(Energy, data.energy - self.energy_mean, torch.ones_like(Energy).bool())
                    E_losses.append(energy_loss.item())
                    E_real.append(data.energy.item() / 96)
                    E_pred.append((Energy.item() + self.energy_mean.item()) / 96)
                if Force is not None:
                    force_loss = criterion(Force, data.force, torch.ones_like(Force).bool())
                    F_losses.append(force_loss.item())
                if Zstar is not None:
                    loss += criterion(Zstar, data.zstar, torch.ones_like(Zstar).bool())
            
            loss = 1*force_loss + 0.02*energy_loss
            ...
            with torch.no_grad():
                ...
                if config.train_target is not None:
                    if Energy is not None:
                        energy_losses.update(energy_loss.item(), output_node.shape[0])
                '''
                if len(self.net_out_info.js) > 1 and not is_train:
                    for i in range(len(self.net_out_info.js)):
                        ...
                        #loss_list[i].update(0, num_hoppings)
                '''    
        ...
        print(f"E_loss: {torch.tensor(E_losses).mean()}, F_loss: {torch.tensor(F_losses).mean()}")
        return losses, loss_list, energy_losses
    ...
    def save_test_result(self, batch, H_pred, h5_fp):
        ...
        if stru_id in h5_fp:
            ...
            g['edge_attr'] = batch.edge_attr.detach()
            g['label'] = batch.label
            g['mask'] = batch.mask
            g['H_pred'] = H_pred
    ...
    def record_train(self):
        ...
        if config.train_target == 'E' or config.train_target == 'EF':
            tb_writer.add_scalars('Loss', {'Energy loss': info.energy_losses.avg}, global_step=info.global_step)
