...
class NNConv_all(torch.nn.Module):
    def __init__(self, edge_feature_l, node_feature_l, hid_l = 64):
        super(Net, self).__init__()
        # 定义边网络，它将边特征转换为权重矩阵
        nn1 = torch.nn.Sequential(torch.nn.Linear(edge_feature_l, hid_l), torch.nn.ReLU(), torch.nn.Linear(hid_l, node_feature_l * node_feature_l))
        self.conv1 = NNConv(node_feature_l, node_feature_l, nn1, aggr='mean')
        nn2 = torch.nn.Sequential(torch.nn.Linear(edge_feature_l, hid_l), torch.nn.ReLU(), torch.nn.Linear(hid_l, node_feature_l))
        self.conv1 = NNConv(node_feature_l, 1, nn2, aggr='mean')

    def forward(self, node_feature, edge_index, edge_feature):
        x = self.conv1(node_feature, edge_index, edge_feature)
        x = torch.relu(x)
        x = self.conv2(node_feature, edge_index, edge_feature)
        return x
...
class Net(nn.Module):
    def __init__(self, num_species, irreps_embed_node, irreps_edge_init, irreps_sh, irreps_mid_node, 
        ...
        #irreps_out_node = irreps_post_node #For developed function
        self.node_to_energy = FullyConnectedTensorProduct(irreps_node_prev, irreps_node_prev, Irreps('64x0e'))
        self.nnconv = NNConv_all(edge_feature_l=num_basis, node_feature_l = 64)
        #self.gatconv1 = GATConv(64, 64)
        #self.gatconv2 = GATConv(64, 1)
        self.nonlinear_layers_for_energy = nn.Sequential(
            ...
            torch.nn.LayerNorm(64, eps=1e-6),
            torch.nn.SiLU(),
            torch.nn.Linear(64, irreps_out_node.dim),
        )
        self.nonlinear_layers_for_energy2 = nn.Sequential(
            torch.nn.Linear(72*64, 64),
            torch.nn.LayerNorm(64, eps=1e-6),
            torch.nn.SiLU(),
            torch.nn.Linear(64, 64),
            torch.nn.LayerNorm(64, eps=1e-6),
    ...
    def forward(self, data):
        ...
        energy_fea = self.node_to_energy(node_fea, node_fea)
        energy_fea = self.nnconv(energy_fea, data["edge_index"], edge_length_embedded)
        print(energy_fea.shape)
        #energy_fea = self.nonlinear_layers_for_energy(energy_fea)
        #energy_fea = self.nonlinear_layers_for_energy2(energy_fea.reshape(-1, 72*64))
        energy_fea = energy_fea.reshape(len(data.energy),-1).sum(dim=-1)
        #edge_fea = self.lin_edge(edge_fea)
        return energy_fea, edge_fea
