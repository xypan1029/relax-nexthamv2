...
class TrainConfig(BaseConfig):
    ...
    def set_target(self, orbital_types, index_to_Z, spinful, output_file):
        ...
        if self.train_target is not None: 
            if self.train_target == 'E' or self.train_target == 'EF':
                self._irreps_post_node = Irreps('1x0e')
            if self.train_target == 'Z':
                self._irreps_post_node = Irreps('1x0e+1x1o+1x2e')
            if self.train_target == 'EFZ':
                self._irreps_post_node = Irreps('1x0e+1x0e+1x1o+1x2e')
        '''
        self._target_set_flag = True
