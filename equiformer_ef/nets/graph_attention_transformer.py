...
class GraphAttention(torch.nn.Module):
    ...
    def forward(self, node_input, node_attr, edge_src, edge_dst, edge_attr, edge_sh, edge_scalars, edge_length, 
        batch, **kwargs):
        ...
        alpha = self.lin_alpha(edge_scalars) #* (torch.exp((-edge_length + 3) / 1.5) * (1 - edge_length / 6)).unsqueeze(1)
...
class TransBlock(torch.nn.Module):
    ...
    def forward(self, node_input, node_attr, edge_src, edge_dst, edge_attr, edge_sh, edge_scalars, 
        ...
        edge_attr = self.norm_edge(edge_attr, batch = batch[edge_src])
        #norm_1_output = node_features
        
        #node_features, _ = self.node_update(node_features, None, edge_sh, edge_attr, 
        #                                  edge_scalars, [edge_src, edge_dst], batch, None, edge_length)

        node_features, edge_attr = self.ga(node_input=node_features, 
            node_attr=node_attr, 
            edge_src=edge_src, edge_dst=edge_dst, 
            edge_attr=edge_attr, edge_sh=edge_sh, edge_scalars=edge_scalars, edge_length=edge_length, 
            batch=batch)
        ...
        #node_output = node_output + node_features # 128x0e+64x1e+32x2e
        #edge_output = edge_output + edge_features
        #edge_features = edge_output
        
        #node_features = node_output
        
        # edge_attr = self.update_edge(node_input=node_features,
        #                                  edge_src=edge_src, edge_dst=edge_dst,
        #                                  edge_attr=edge_attr,
        #                                  edge_sh=edge_sh,
        ...
        # _, edge_attr = self.ga_edge(node_input=node_features, 
        #     node_attr=node_attr, 
        #     edge_src=edge_src, edge_dst=edge_dst, 
        ...
        #edge_attr = self.edge_update(node_features, None, edge_sh, edge_attr, edge_scalars, [edge_src, edge_dst], batch)
        # print(self.irreps_edge_output)
        # print(edge_attr[1])
        #node_features = self.concat_norm_output(norm_1_output, node_features)
         # include Gate
        # #edge_base = torch.ones_like(edge_features.narrow(1, 0, 1))
...
class ScaledScatter_xypan(torch.nn.Module):
    def __init__(self, avg_aggregate_num):
        super().__init__()
        self.avg_aggregate_num = avg_aggregate_num + 0.0
    ...
    def forward(self, x, index, dim, dim_size):
        ...
        print(edge_features[index == index[0]].sum())
        print(edge_features[index == index[1]].sum())
        print(edge_features[index == index[2]].sum())
        num_nodes = torch.max(index) + 1
        node_features = torch.zeros(num_nodes, device = edge_features.device)
        for i in range(len(index)):
            node_features[index[i]] += edge_features[i]
        print(node_features)
        exit()
        out = node_features.index_add_(0, index, edge_features)
        # out = out.div(self.avg_aggregate_num ** 0.5)
        return out
