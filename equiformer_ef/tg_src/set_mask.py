def get_graph(cart_coords, frac_coords, numbers, elements, stru_id, r, max_num_nbr, edge_Aij, lattice,
    ...
    if data_folder is not None:
        if inference:
            data = Data(x=numbers, edge_index=edge_idx, edge_attr=edge_fea, stru_id=stru_id,
                        ...
                        spinful=spinful, name=data_folder,
                        atoms=elements,
                        **kwargs)
        else:
            if edge_Aij:
                ...
                for index in range(edge_fea.shape[0]):
                    i, j = edge_idx[:, index] # i=src, j=dst
